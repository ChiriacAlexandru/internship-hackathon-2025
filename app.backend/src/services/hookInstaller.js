 ...
 */
export function installPreCommitHook(repoPath, projectId, authToken, apiUrl = 'http://localhost:3000') {
  try {
    // Validate repo path
    ...
    const templatePath = path.join(__dirname, '..', 'scripts', 'pre-commit-hook.js');
    let hookContent = fs.readFileSync(templatePath, 'utf-8');

    // Replace placeholders
    hookContent = hookContent.replace('REPLACE_API_URL', apiUrl);
    hookContent = hookContent.replace('REPLACE_PROJECT_ID', projectId.toString());
