...
const RepoLinkPage = () => {
    ...
    const loadData = async () => {
      try {
        ...
        const hookStatusPromises = repos
          ...
          .map(async (proj) => {
            try {
              const hookData = await apiClient(
                `/me/repos/${proj.projectId}/hook-status`,
                { token }
              );
              return {
                projectId: proj.projectId,
                installed: hookData?.installed ?? false,
              };
            } catch {
  ...
  const handleSave = async (projectId) => {
    ...
    setStatusMap((prev) => ({
      ...prev,
      [projectId]: { type: "loading", message: "" },
    }));
    ...
    try {
      ...
      setStatusMap((prev) => ({
        ...
        [projectId]: {
          type: "success",
          message: payload?.message ?? "Repository linked successfully.",
        },
      }));
    } catch (error) {
      setStatusMap((prev) => ({
  ...
  const handleClear = async (projectId) => {
    ...
    try {
      ...
      setStatusMap((prev) => ({
        ...
        [projectId]: {
          type: "success",
          message: payload?.message ?? "Repository link removed.",
        },
      }));
    } catch (error) {
      setStatusMap((prev) => ({
  ...
  const handleInstallHook = async (projectId) => {
    ...
    try {
      ...
      setStatusMap((prev) => ({
        ...
        [projectId]: {
          type: "success",
          message: payload?.message ?? "Pre-commit hook installed!",
        },
      }));
    } catch (error) {
      setStatusMap((prev) => ({
  ...
  const handleUninstallHook = async (projectId) => {
    ...
    try {
      ...
      setStatusMap((prev) => ({
        ...
        [projectId]: {
          type: "success",
          message: payload?.message ?? "Pre-commit hook removed!",
        },
      }));
    } catch (error) {
      setStatusMap((prev) => ({
    ...
    <div className="space-y-8">
      <section>
        ...
        <p className="mt-2 text-sm text-slate-400">
          Configure the local Git path for each project you are assigned to. The
          platform validates that the directory exists and contains a{" "}
          <code className="rounded bg-slate-800 px-1">.git</code> folder before
          saving.
        </p>
      </section>
          ...
          <tbody className="divide-y divide-slate-800">
            {loading ? (
              ...
              projects.map((project) => {
                ...
                const status = statusMap[projectId] ?? {
                  type: "idle",
                  message: "",
                };
                  ...
                  >
                    ...
                    <td className="px-4 py-4">
                      <input
                        ...
                        value={inputValue}
                        onChange={(event) =>
                          handleInputChange(projectId, event.target.value)
                        }
